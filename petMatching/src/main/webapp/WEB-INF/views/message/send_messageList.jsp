...
.receiver{display:inline-block; width:15%}
...
<div class="container">
	...
	<div class="container">
	 <div class="container">
		<p class="text-danger">
		  <font size = 4>보낸 메시지함</font>&nbsp;&nbsp;&nbsp;
		  <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
		  <em id="listcount" class="text-danger">보낸 메시지 수 : ${sendMessageCount} </em>
		</p>
		<hr class="text-danger">
  
  <div class="sesarch">
  <form action="sendMessageList" >
    <div class="input-group">
      <select id="viewcount" name="search_field">
        <option value="R" selected>받는이</option>
      ...
      <input name="search_word" type="text" class="form-control"
              placeholder="받는이의 id를 입력하세요." value="${search_word}" style="border:1px solid #dc3545; height:30px">
       </div> &nbsp;
     	 <button id="search_button" type="submit"><img src="${pageContext.request.contextPath}/resources/image/search.png" width="15px"></button>        
    </div>
  </form>
</div>
  ...
  <c:if test="${sendMessageCount >0}"> 
    &nbsp;&nbsp;&nbsp;&nbsp;<span class="receiver"><b>받는이</b></span><span class="title"><b>제목</b></span>
    <span class="date"><b>보낸 날짜</b></span><span class="read">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>메시지 상태</b></span><hr class="text-danger">
    <c:forEach var="s" items="${sendMessageList}">
      <input type="checkbox" name="select_delete" value="${s.message_num}">
      <a href="detailSendMessage?num=${s.message_num}">
      <span class="receiver" style="color:black;">${s.receiver_id}</span><span class="title" style="color:black;">${s.message_title}</span><span class="date" style="color:black;">${s.register_date}</span>
      <c:if test="${s.read_count >0 && s.cancelby_sender=='n'}">
        <span class="read" style="color:black;">
      ...
      <c:if test="${s.cancelby_sender=='y'}">
        <span class="read" style="color:black;">
          <img src="${pageContext.request.contextPath}/resources/image/cancel_message.png" alt="메시지 발송 취소" width="30px">메시지 발송 취소
        </span>
      </c:if>
      </a><hr class="text-danger">
    </c:forEach>
  ...
  <div>
    <ul class="pagination justify-content-center">
      ...
      <c:if test="${page >1 }">
        <li class="page-item">
          <%-- 아래 주소 띄어쓰면 안 됨 --%>
          <a href="sendMessageList?page=${page-1}&search_field=${search_field}&search_word=${search_word}"    
      ...
      <c:forEach var="a" begin="${startpage}" end="${endpage}">
        ...
        <c:if test="${a !=page }">
          <li class="page-item">
            <%-- 아래 주소 띄어쓰면 안 됨 --%>
            <a href="sendMessageList?page=${a}&search_field=${search_field}&search_word=${search_word}"
               class="page-link" >${a}</a>
          </li>
        </c:if>
      </c:forEach>
...
<%-- 받은 메시지가 없는 경우 --%>
<c:if test="${sendMessageCount == 0 && empty search_word}">
  <br><br><br><h2>보낸 메시지가 없습니다.</h2><br><br><br>
</c:if>

<%-- 검색 결과가 없는 경우 --%>
<c:if test="${sendMessageCount == 0 && !empty search_word}">
  <br><br><br><h2 style="color:#dc3545">검색 결과가 없습니다.</h2><br><br><br>
</c:if>

<div class="container">
  <div class="input-group">
  
  <form action="deleteBySender" method="post" id="delete_data">
